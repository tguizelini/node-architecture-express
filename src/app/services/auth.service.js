const UseRepository = require('../repositories/auth.repository')
...
class AuthService {
  async auth (email, password) {
    const user = await UseRepository.findOne({ email }).select('+password')
  ...
  async register (name, email, password) {
    const checkExists = await UseRepository.findOne({ email })

    if (checkExists) return HttpResponse.forbiden(null, 'User already exists')

    const user = await UseRepository.create({ name, email, password })
    user.password = undefined
